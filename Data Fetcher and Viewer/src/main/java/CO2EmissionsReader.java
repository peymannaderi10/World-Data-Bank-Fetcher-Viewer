...
public class CO2EmissionsReader extends Reader {
     ...
     * @return a hash map that maps years to CO2 emissions (metric tons per capita).
     */
    public HashMap<Integer, Float> fetchData(String countryCode, Integer startYear, Integer endYear) {
        ...
        String urlString = String.format(
                "http://api.worldbank.org/v2/country/%s/indicator/EN.ATM.CO2E.PC?date=%d:%d&format=json",
                countryCode,
                startYear,
                endYear
        );

        float emissionsForYear = 0; // The CO2 emissions for a particular year.
        HashMap<Integer, Float> data = new HashMap<>(); // The return value.
            ...
            if (responseCode == 200) {
                ...
                for (int i = 0; i < sizeOfResults; i++) {
                    ...
                    if (jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").isJsonNull()) {
                        ...
                        emissionsForYear = jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").getAsFloat();
                        data.put(year, emissionsForYear);
                    }
                }
            }
        } catch (IOException e) {
