...
/**
 * The reader class that fetches data from the World Bank data repository pertaining to energy use.
 */
public class EnergyUseReader extends Reader {
    /**
     ...
     * @return a hash map that maps years to energy use (kg of oil equivalent per capita).
     */
    public HashMap<Integer, Float> fetchData(String countryCode, Integer startYear, Integer endYear) {
        ...
        String urlString = String.format(
                "http://api.worldbank.org/v2/country/%s/indicator/EG.USE.PCAP.KG.OE?date=%d:%d&format=json",
                countryCode,
                startYear,
                endYear
        );

        float energyUseForYear = 0;
            ...
            if (responseCode == 200) {
                ...
                for (int i = 0; i < sizeOfResults; i++) {
                    ...
                    if (jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").isJsonNull()) {
                        ...
                        energyUseForYear = jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").getAsFloat();
                        data.put(year, energyUseForYear);
                    }
                }
            }
        } catch (IOException e) {
