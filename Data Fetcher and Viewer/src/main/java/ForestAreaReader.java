 ...
 * The reader class that fetches data from the World Bank data repository pertaining to forest area.
 */
public class ForestAreaReader extends Reader {
    /**
     ...
     * @return a hash map that maps years to forest area (% of land area).
     */
    public HashMap<Integer, Float> fetchData(String countryCode, Integer startYear, Integer endYear) {
        ...
        String urlString = String.format(
                "http://api.worldbank.org/v2/country/%s/indicator/AG.LND.FRST.ZS?date=%d:%d&format=json",
                countryCode,
                startYear,
                endYear
        );

        float forestAreaPercentageForYear = 0;
            ...
            if (responseCode == 200) {
                ...
                for (int i = 0; i < sizeOfResults; i++) {
                    ...
                    if (jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").isJsonNull()) {
                        ...
                        forestAreaPercentageForYear = jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").getAsFloat();
                        data.put(year, forestAreaPercentageForYear);
                    }
                }
            }
        } catch (IOException e) {
