 ...
 * The reader class that fetches data from the World Bank data repository pertaining to government expenditure on education.
 */
public class GovernmentExpenditureReader extends Reader {
    /**
     ...
     * @return a hash map that maps years to government expenditure on education (total % of GDP).
     */
    public HashMap<Integer, Float> fetchData(String countryCode, Integer startYear, Integer endYear) {
        ...
        String urlString = String.format(
                "http://api.worldbank.org/v2/country/%s/indicator/SE.XPD.TOTL.GD.ZS?date=%d:%d&format=json",
                countryCode,
                startYear,
                endYear
        );

        float expenditureForYear = 0;
            ...
            if (responseCode == 200) {
                ...
                for (int i = 0; i < sizeOfResults; i++) {
                    ...
                    if (jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").isJsonNull()) {
                        ...
                        expenditureForYear = jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").getAsFloat();
                        data.put(year, expenditureForYear);
                    }
                }
            }
        } catch (IOException e) {
