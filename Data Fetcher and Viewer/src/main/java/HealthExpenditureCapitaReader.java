...
/**
 * The reader class that fetches data from the World Bank data repository pertaining to current health expenditure per capita.
 */
public class HealthExpenditureCapitaReader extends Reader {
    /**
     ...
     * @return a hash map that maps years to current health expenditure per capita (current US dollars).
     */
    public HashMap<Integer, Float> fetchData(String countryCode, Integer startYear, Integer endYear) {
        ...
        String urlString = String.format(
                "http://api.worldbank.org/v2/country/%s/indicator/SH.XPD.CHEX.PC.CD?date=%d:%d&format=json",
                countryCode,
                startYear,
                endYear
        );

        float healthExpenditurePerCapitaForYear = 0;
            ...
            if (responseCode == 200) {
                ...
                for (int i = 0; i < sizeOfResults; i++) {
                    ...
                    if (jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").isJsonNull()) {
                        ...
                        healthExpenditurePerCapitaForYear = jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").getAsFloat();
                        data.put(year, healthExpenditurePerCapitaForYear);
                    }
                }
            }
        } catch (IOException e) {
