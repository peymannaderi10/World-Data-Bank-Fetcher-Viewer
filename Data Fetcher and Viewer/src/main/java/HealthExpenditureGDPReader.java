...
/**
 * The reader class that fetches data from the World Bank data repository pertaining to current health expenditure (% of GDP).
 */
public class HealthExpenditureGDPReader extends Reader {
    /**
     ...
     * @return a hash map that maps years to current health expenditure (% of GDP).
     */
    public HashMap<Integer, Float> fetchData(String countryCode, Integer startYear, Integer endYear) {
        ...
        String urlString = String.format(
                "http://api.worldbank.org/v2/country/%s/indicator/SH.XPD.CHEX.GD.ZS?date=%d:%d&format=json",
                countryCode,
                startYear,
                endYear
        );

        float healthExpenditureForYear = 0;
            ...
            if (responseCode == 200) {
                ...
                for (int i = 0; i < sizeOfResults; i++) {
                    ...
                    if (jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").isJsonNull()) {
                        ...
                        healthExpenditureForYear = jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").getAsFloat();
                        data.put(year, healthExpenditureForYear);
                    }
                }
            }
        } catch (IOException e) {
