 ...
 * The reader class that fetches data from the World Bank data repository pertaining to hospital beds.
 */
public class HospitalBedsReader extends Reader {
    /**
     ...
     * @return a hash map that maps years to hospital beds (per 1,000 people).
     */
    public HashMap<Integer, Float> fetchData(String countryCode, Integer startYear, Integer endYear) {
        ...
        String urlString = String.format(
                "http://api.worldbank.org/v2/country/%s/indicator/SH.MED.BEDS.ZS?date=%d:%d&format=json",
                countryCode,
                startYear,
                endYear
        );

        float hospitalBedsForYear = 0;
            ...
            if (responseCode == 200) {
                ...
                for (int i = 0; i < sizeOfResults; i++) {
                    ...
                    if (jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").isJsonNull()) {
                        ...
                        hospitalBedsForYear = jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").getAsFloat();
                        data.put(year, hospitalBedsForYear);
                    }
                }
            }
        } catch (IOException e) {
