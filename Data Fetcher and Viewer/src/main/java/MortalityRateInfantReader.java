...
/**
 * The reader class that fetches data from the World Bank data repository pertaining to mortality rate for infants.
 */
public class MortalityRateInfantReader extends Reader {
    /**
     ...
     * @return a hash map that maps years to mortality rate for infants (per 1,000 live births).
     */
    public HashMap<Integer, Float> fetchData(String countryCode, Integer startYear, Integer endYear) {
        ...
        String urlString = String.format(
                "http://api.worldbank.org/v2/country/%s/indicator/SP.DYN.IMRT.IN?date=%d:%d&format=json",
                countryCode,
                startYear,
                endYear
        );

        float mortalityRateForYear = 0;
            ...
            if (responseCode == 200) {
                ...
                for (int i = 0; i < sizeOfResults; i++) {
                    ...
                    if (jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").isJsonNull()) {
                        ...
                        mortalityRateForYear = jsonArray.get(1).getAsJsonArray().get(i).getAsJsonObject().get("value").getAsFloat();
                        data.put(year, mortalityRateForYear);
                    }
                }
            }
        } catch (IOException e) {
